...
class installModel
{
     ...
     * @return string
     * @author ljj
     * @date 2024/1/5 6:46 下午
     */
    public function checkMysql()
    {
        $useVersion = mysqli_get_client_version();
        $astrictVersion = explode('.','5.7.0');
        $astrictVersion = $astrictVersion[0] * 10000 + $astrictVersion[1] * 100 + $astrictVersion[2];
        return $useVersion >= $astrictVersion ? 'ok' : 'fail';
    }
...
//            }
    ...
    public function initAccount($post)
    {
        ...
        // 超级管理员
        $sql = "INSERT INTO `ls_admin`(`id`, `root`, `name`, `avatar`, `account`, `password`, `login_time`, `login_ip`, `multipoint_login`, `disable`, `create_time`, `update_time`, `delete_time`) VALUES (1, 1, '{$post['admin_user']}', '', '{$post['admin_user']}', '{$password}','{$time}', '', 1, 0, '{$time}', '{$time}', NULL);";
        // 超级管理员关联部门
        $sql .= "INSERT INTO `ls_admin_dept` (`admin_id`, `dept_id`) VALUES (1, 1);";
